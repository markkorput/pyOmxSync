...
class Receiver:
    ...
    def update(self):
        ...
        self.logger.debug('PositionReceiver got: %s @ %.2f (deviation: %.2f)' % (self.received_filename, self.received_position, self.deviation))
        ...
        if self.file_must_match and not os.path.basename(self.received_filename) == os.path.basename(self.player.get_filename()):
            self.logger.info('PositionReceiver got different file ({0}) from own current file: {1}'.format(os.path.basename(self.received_filename), os.path.basename(self.player.get_filename())))
            return

        # calculate median deviation
        self.deviations.append(self.deviation)
        self.median_deviation = self._calculate_median(list(self.deviations))
        self.logger.debug('median_deviation: {0}'.format(self.median_deviation))
    ...
    def _perform_sync(self):
        ...
        if self.deviation < 0 and abs(self.deviation) < self.jump_ahead:
            ...
            self.logger.debug("paused for %.2f seconds" % (-self.deviation))
            return
        ...
        self.logger.debug("jumped to position %.2f and paused for %.2f seconds" % (pos, self.jump_ahead))
